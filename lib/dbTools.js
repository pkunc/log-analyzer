...
function* createViews(db) {
  try {
    // const result = (yield promisify(cloudant.get))[0];
    ...
    console.log(`[db.views] Views to create: ${view._id}`);
    const result = yield db.insert(view);
    console.log(chalk.green(`[db.views] Views created: ${JSON.stringify(result)}`));
  } catch (err) {
